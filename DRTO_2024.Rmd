...
University of Miami Cooperative Institute of Marine and Atmospheric Science (CIMAS)<br>


<br>

<h5 style="font size=6">
N. Besemer, A. Palacio, M. Jankulak, A. Webb, I. Enochs - October 2024
</h5>
...
The	AOML	Coral	Program	tracks	the	status	and	trends	of coral	reef	ecosystems	in	the	U.S.	Atlantic	and	Caribbean	 as	part	of	the	National	Coral	Reef	Monitoring	Program	(NCRMP).	This	summary	brief	provides	an	overview	of the	most	recent	climate	monitoring	efforts	in Dry Tortugas National Park


### Expedition summary

* The NCRMP Atlantic climate monitoring team surveryed in Dry Tortugas National Park from June 23rd to June 27th, 2024

* A total of Four different sites were visited by 6 team members completing a total of 76 dives. 
...
**Subsurface temperature:** Subsurface temperature	recorders	(STRs)	were	recovered	and	redeployed	at	four sites,	each representing four different depths	(Fig.	1).	In	total,	more	than	10	million	temperature	observations	were	collected	from 4 instruments	(Table	1).
...
**NCRMP	Climate	fixed	sentinel	site	monitoring:** At Bird Key reef	-	15m	site,	located	in	SE	DRTO,	short	term instruments	(54h)	were	deployed	to	monitor	daily	fluctuations	in:
CHECK NUMBERS 
...
The	temperatures	that	marine	organisms	experience	are	a function	of	local	oceanographic	conditions	and	vary	with	depth. To	monitor	subsurface temperature, four sites were established arounf the national park representing four depths (1,	5,	15,	25m;	Fig.2). Temperature was measured	using	SeaBird	Electronics	Subsurface	Temperature Recorders	(STR)s	that	collected	data	at	5-minute	intervals.	These instreuments are recovered and replaced every 3 years since 2015. Gaps exsist in the data as not all instruments survived the deployment time period, likely due to battery issues. DO WE WANT TO INCLUDE OLDER DATA OR ELABORATE ON CUT OFF?
...
<font size="2"> **Figure 2:** Temperature conditions at four sites in the Dry Tortugas representing a depth gradient: Pulaski Shoal Lighthouse (1m) White Shoal (5m), Bird Key Reef (15m), and Black Coral Rock (25m). Data were collected from June 2021 to 2024, but instruments failed at XX, XX XXX. </font>
...
```
UPDATE THIS SUMMARY STATS FOR TEMP
...
Seawater	carbonate	chemistry	can	fluctuate	diurnally,	due	to biological	forcing	processes	such	as	photosynthesis	and	respiration,	as	well	as	calcification	and	dissolution.	To characterize	this,	discrete	water	samples	(Fig.	3)	were collected	at	three-hour	intervals	(n=15)	using	Subsurface Automatic	Samplers	(SAS,	www.coral.noaa.gov/accrete/sas). These	samples	will	be	analyzed	for	Total	Alkalinity	(TA),	Dissolved	Inorganic	Carbon	(DIC),	and	Spectrophotometric pH	(Spec pH),	which	will	be	used	to	calculate	pCO2 and	aragonite	saturation	state	(ΩAragonite).	A	suite	of	instruments	was deployed	for	a	72-hour	period	at Bird Key	15m	site.	A	SAMI pH was	used	to	log	pH,	an	EcoPAR	measured Photosynthetically	Active	Radiation	(PAR),	and	a	Lowell Tiltmeter	measured	current	speed	and	direction.	Each collected	measurements	at	15-minute	intervals	(Fig.	3).
...
<font size="2">**Figure 3:** Data from Bird Key Reef (15m) diurnal suite monitoring from June 23th to 27th, 2024. Top	panel:	pH	and	temperature	from SAMI pH.	Bottom	panel:	Photosynthetically	Available	Radiation	(PAR)	and	current	speed	from	EcoPAR	and	Tiltmeter.	Shading	
denotes	nighttime	throughout	sequence	of	the	plot.	Instruments	measured	parameters	every	15	minutes.	 </font>
...
**Carbonate budget** assessments use transect-based surveys (ReefBudget) to quantify the abundance of carbonate producers( e.g., corals and crustose coralline algae), and carbonate bioeroders	(e.g.,	parrotfish	and	sea	urchins). Abundances are multiplied by taxon- specific rates of carboante alteration to determine if a reef is in a state of net accretion (habitat growth) or net loss (habitat loss; Fig.4). At Bird Key Reef, six transects were established and surveyed in 2015, 2018, 2021 and 2024 to obtain carbonate budgets.  The transect results showed that carbonate budgets have decreased in 2024. This trend is driven by a reduction in coral cover and calcification as well as a steep increase in parrotfish erosion. MORE SUMMAITON TEXT FOR DRTO CARB BUDGETS
...
<font size="2">**Figure 4:** Net carbonate production and the magnitude of carbonate alteration by calcifying or bioeroding functional	groups.	CCA represents crustose coralline algae. PF	represents	parrotfish. The	scale	of the	y-axis varies for
each	functional	group. </font>
...
**Calcification Accretion Units (CAUs)** and **Bioerosion Monitoring Units (BMUs)** were	used	to investigate the balance between calcification and
erosion. CAUs and BMUs were collected and redeployed for the next sampling cycle. CAUs are processed by the NCRMP Pacific Climate group and the data will	be available within a year. BMUs will	be dried and cleaned using a hydrogen peroxide solution. Samples will	be weighed and scanned using a	CT	scanner and	then	compared to	their	pre-scans	to	quantify bioerosion.
...
**Field team members**: Ana Palacio, Ian Enochs, Nicole Besemer, Rich Karp, Taylor Gill, Ashley Stevens
