...
The NOAA Atlantic Oceanographic and Meteorolgical Laboratory (AOML) conducts the long-term [National Coral Reef Monitoring Program (NCRMP)](https://www.coris.noaa.gov/monitoring/welcome.html) to track the status and trends of coral reef ecosystems of the U.S. Atlantic and Caribbean coral reef jurisdictions. This FY21 summary brief provides an overview of the most recent survey efforts.
...
* Four different sites were monitored by collecting and deploying instruments that monitor temperature, current, pH, light, bioerosion and calcification
...
Subsurface Temperature Recorder (STR)s were placed at all 4 sites and collected temperature measurements at 5-minute intervals for 3 years. At Bird Key Reef additional instruments were deployed for a 72-hour diurnal suite. SeaFET pH logger, Tiltmeter and EcoPAR collected measurements at 5-minute intervals. Subsurface Automatic Samplers (SAS) collected discrete water samples at three-hour intervals (n=24).
...
```{r,echo=FALSE, warning=FALSE,fig.width = 12,fig.height = 14, eval= FALSE}
...
figure2a<-ggarrange(phline, templine, currentline, PARline, ncol=1)
figure2a
```
...
figure2b<-ggarrange(phtempplot, parcurrentplot, ncol=1)
figure2b

```


<font size="2"> **Figure 2:**  Summary plots of pH paired with Temperature, and  current speed paired with PAR collected from instrument deployment at Bird Key Reef site around 15 meters from June 25th to June 28th. Instruments measured parameters every 5 minutes. </font>


option 2:


```{r ph and temp combined,echo=FALSE, warning=FALSE,fig.width = 12,fig.height = 6}
...
ggplot(data = pH) +
  ...
  annotate("text",
          ...
          x = seq.POSIXt(as.POSIXct('2021-06-25 13:30:00', tz='EST'),
                         by = "day", length.out = 4),
          y = 7.985)+ 
  scale_color_manual(values=c("darkorange", "coral1"))
...
ggplot(data = light) +
  ...
  annotate("text",
          ...
          x = seq.POSIXt(as.POSIXct('2021-06-25 13:30:00', tz='EST'),
                         by = "day", length.out = 4),
          y = 450)+ 
  scale_color_manual(values=c("dodgerblue", "aquamarine4"))
...
pH | number| number| number| 1 depth 
Light| number| number| number| 1 depth
Current| number| number| number| 1 depth
...
![New STR depolyed at White Shoal in Dry Tortugas National Park](Data/STRfish.jpg)
