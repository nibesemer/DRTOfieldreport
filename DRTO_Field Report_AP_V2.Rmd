...
The NOAA Atlantic Oceanographic and Meteorolgical Laboratory (AOML) conducts the long-term [National Coral Reef Monitoring Program (NCRMP)](https://www.coris.noaa.gov/monitoring/welcome.html) to track the status and trends of coral reef ecosystems of the U.S. Atlantic and Caribbean coral reef jurisdictions. This FY21 summery brief provides an overview of the most recent survey efforts.
...
phtempplot<- ggplot() +
  geom_line(data = pH, aes(x=DateTime, y=pH),color = "dark orange", size=1) +
  geom_line(data = pH, aes(x=DateTime, y=Temp.adj), color = "coral1", size=1, alpha = 0.5) +
  theme(plot.margin = unit(c(5.5, 15, 5.5, 5.5), "points")) +
  scale_x_datetime(breaks = date_breaks("8 hours"),
                  labels = date_format("%m/%d\n%H:%M",
                                        tz="America/New_York"),
  ...
  xlab(expression(atop("Date and Time (EDT)"))) +
  scale_y_continuous(limits = c(7.925,7.985), n.breaks = 5, sec.axis = sec_axis(~(.-4.8371)/0.1099, name="Temperature (\u00B0C)")) 
...
parcurrentplot<- ggplot() +
  ...
  ylab(expression(atop(paste("PAR (",mu,"mol photons m"^-2,"s"^-1,")")))) +
  xlab(expression(atop("Date and Time (EDT)"))) +
  scale_y_continuous(limits = c(0,500), sec.axis = sec_axis(~./scaleFactor2, name="Current Speed (cm/s)", breaks = seq(0,12,2)))
...
Temperature records: 1.1 million points, 700 days, 4 depths
pH measurements: 
Light measurements:
